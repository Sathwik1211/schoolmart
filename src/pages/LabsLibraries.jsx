        ...
        <section className="py-8 border-t border-gray-100 flex flex-col lg:flex-row gap-8">
           <aside className="lg:w-[240px] flex-shrink-0">
              <div className="sticky top-24 space-y-2">
                 ...
                 {['SCIENCE LABS', 'MODERN LIBRARY', 'STEM KITS', 'MAKER SPACE', 'ART & DESIGN'].map((cat, i) => (
           ...
           <div className="flex-grow">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-start">
                 {compositeItems.map((work, i) => (
                    <React.Fragment key={i}>
                       <div 
                         className={`relative overflow-hidden rounded-[25px] shadow-sm group cursor-pointer border border-gray-300 h-[220px] transition-all duration-500 bg-gray-50 ${selectedItem?.title === work.title ? 'ring-4 ring-emerald-600 shadow-2xl scale-[1.02]' : 'hover:scale-[1.01]'}`}
                       ...
                       {i % 2 === 1 && (
                          <div className="hidden md:block lg:hidden col-span-full">
                             {compositeItems.slice(i-1, i+1).some(dw => dw.title === selectedItem?.title) && (
                                <InlineQuickView isOpen={true} onClose={() => setSelectedItem(null)} data={selectedItem} />
                             )}
                          </div>
                       )}
                       ...
                       {i % 3 === 2 && (
                          <div className="hidden lg:block col-span-full">
                             {compositeItems.slice(i-2, i+1).some(dw => dw.title === selectedItem?.title) && (
                                <InlineQuickView isOpen={true} onClose={() => setSelectedItem(null)} data={selectedItem} />
                             )}
                          </div>
                       )}
                       {/* Handle End of List */}
                       {i === compositeItems.length - 1 && (
                          <>
                             <div className="hidden md:block lg:hidden col-span-full">
                                {compositeItems.slice(Math.floor(i/2)*2).some(dw => dw.title === selectedItem?.title) && i % 2 !== 1 && (
                                   <InlineQuickView isOpen={true} onClose={() => setSelectedItem(null)} data={selectedItem} />
                                )}
                             </div>
                             <div className="hidden lg:block col-span-full">
                                {compositeItems.slice(Math.floor(i/3)*3).some(dw => dw.title === selectedItem?.title) && i % 3 !== 2 && (
                                   <InlineQuickView isOpen={true} onClose={() => setSelectedItem(null)} data={selectedItem} />
                                )}
                             </div>
                          </>
